...
public class kingMovesCalculator implements moveCalculator {
    ...
    public Collection<ChessMove> possibleMoves(ChessBoard board, ChessPosition myPosition) {
        ...
        calculate(board,myPosition,moves,myTeam,myRow+0,myCol+1);
        calculate(board,myPosition,moves,myTeam,myRow+1,myCol+1);
        calculate(board,myPosition,moves,myTeam,myRow-1,myCol+1);
        calculate(board,myPosition,moves,myTeam,myRow+0,myCol-1);
        calculate(board,myPosition,moves,myTeam,myRow+1,myCol-1);
        calculate(board,myPosition,moves,myTeam,myRow-1,myCol-1);
        calculate(board,myPosition,moves,myTeam,myRow+1,myCol+0);
        calculate(board,myPosition,moves,myTeam,myRow-1,myCol+0);
        return moves;
    }
}
